...
df_baskets["Ergebnis"] = ""  # wichtig für Filterlogik
  ...
  <script>
    function filter() {{
      ...
      document.querySelectorAll("#spielplan tbody tr").forEach(row => {{
        ...
        const isVisible = matchTeam && matchRunde && matchOrt && matchWeek && withinTime;
        row.style.display = isVisible ? "" : "none";
        if (isVisible) visibleRows++;
      }});

      // Wenn ohne "Vergangene Spiele" keine Zeilen sichtbar sind,
      // automatisch auf komplette Saisonansicht umschalten.
      if (!week && !showPast && visibleRows === 0) {{
